...
-- Nécessite un exploit avec writefile (ex: Synapse X, KRNL, etc.)

local function scanInstance(instance, indent)
	indent = indent or ""
	local info = indent .. instance.ClassName .. ": " .. instance.Name .. "\n"
	-- Si c'est un script, on tente de lire son contenu
	if instance:IsA("LocalScript") or instance:IsA("Script") or instance:IsA("ModuleScript") then
		pcall(function()
			info = info .. indent .. "  [Script Source]:\n"
			info = info .. indent .. "  " .. instance.Source:gsub("\n", "\n" .. indent .. "  ") .. "\n"
		end)
	end
	-- Parcours récursif des enfants
	for _, child in ipairs(instance:GetChildren()) do
		info = info .. scanInstance(child, indent .. "  ")
	end
	return info
end

-- Récupère le nom du jeu
local gameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name or "UnknownGame"
local fileName = gameName:gsub("[^%w]", "_") .. ".txt"

-- Scan complet du jeu
local scanResult = "Scan du jeu: " .. gameName .. "\n\n"
scanResult = scanResult .. scanInstance(game)

-- Écrit dans un fichier .txt
writefile(fileName, scanResult)

print("Scan terminé. Fichier généré: " .. fileName)
