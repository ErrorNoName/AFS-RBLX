...
local function validateKey(key)
    ...
    return true, {code = "KEY_VALID", data = {auth_expire = -1}}
end
...
CheckButton.MouseButton1Click:Connect(function()
    ...
    createNotification("Success", "Your key is valid! Lifetime key", 5, Color3.fromRGB(85, 255, 127), ScreenGui)

    -- Sauvegarder la clé
    pcall(function() writefile("Key.txt", keyText) end)

    -- Exécuter le script si auto-execute activé
    if autoExecuteEnabled then
        local success, err = executeScript(keyText)
        if not success then
            createNotification("Script Error", tostring(err), 5, Color3.fromRGB(255, 85, 85), ScreenGui)
            return
        end
